...
# 光照参数（用 field 以便 Kernel 中读取 GUI 实时更新的值）
...
BG_COLOR  = tm.vec3(0.05, 0.2, 0.25)  # 深青色背景

# 球体（左侧）
SPHERE_CENTER = tm.vec3(-1.2, -0.2, 0.0)
SPHERE_RADIUS = 1.2
SPHERE_COLOR  = tm.vec3(0.8, 0.1, 0.1)  # 深红
...
def intersect_cone(ray_o: tm.vec3, ray_d: tm.vec3) -> float:
    """
    圆锥：顶点 CONE_APEX，向 -Y 延伸，底面在 y = CONE_BASE_Y。
    侧面方程：(x-ax)^2+(z-az)^2 = k^2*(ay-y)^2，k = R/H
    另外单独处理底面圆盖。
    """
...
def phong_shade(hit: tm.vec3, N: tm.vec3, obj_color: tm.vec3) -> tm.vec3:
    ...
    R = (2.0 * tm.dot(N, L) * N - L).normalized()  # 理想反射方向
...
def main():
    window = ti.ui.Window("Phong Lighting Model", (WIDTH, HEIGHT), vsync=True)
