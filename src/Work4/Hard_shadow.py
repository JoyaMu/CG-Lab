...
# ────────────────────────────────────────────────
# 暗影射线检测
# 从命中点向光源发射 Shadow Ray，判断是否被遮挡
# t_light：命中点到光源的距离，遮挡物的 t 必须小于此值才算有效遮挡
# ────────────────────────────────────────────────

@ti.func
def in_shadow(hit: tm.vec3) -> bool:
    shadow_o = hit                               # 起点：命中点（偏移已由 1e-4 阈值处理）
    shadow_d = (LIGHT_POS - hit).normalized()    # 方向：指向光源
    t_light  = (LIGHT_POS - hit).norm()          # 到光源的距离

    blocked = False

    t_s = intersect_sphere(shadow_o, shadow_d)
    if t_s > 1e-4 and t_s < t_light:
        blocked = True

    t_c = intersect_cone(shadow_o, shadow_d)
    if t_c > 1e-4 and t_c < t_light:
        blocked = True
...
# ────────────────────────────────────────────────
# Phong 着色器（含硬阴影）
# 阴影区域只保留 Ambient，Diffuse 和 Specular 置零
# ────────────────────────────────────────────────

@ti.func
def phong_shade(hit: tm.vec3, N: tm.vec3, obj_color: tm.vec3) -> tm.vec3:
    ka   = Ka[None]
    kd   = Kd[None]
    ks   = Ks[None]
    shin = Shininess[None]
    ...
    color = ambient  # 阴影中只有环境光

    if not in_shadow(hit):
        L = (LIGHT_POS - hit).normalized()
        V = (CAM_POS   - hit).normalized()
        R = (2.0 * tm.dot(N, L) * N - L).normalized()

        diffuse  = kd * ti.max(0.0, tm.dot(N, L)) * LIGHT_COL * obj_color
        specular = ks * tm.pow(ti.max(0.0, tm.dot(R, V)), shin) * LIGHT_COL
...
def render():
    for i, j in pixels:
        ...
        if hit_obj == 1:
            ...
            N     = sphere_normal(hit)
            color = phong_shade(hit, N, SPHERE_COLOR)
        elif hit_obj == 2:
            hit   = ray_o + best_t * ray_d
            N     = cone_normal(hit)
            color = phong_shade(hit, N, CONE_COLOR)
...
def main():
    window = ti.ui.Window("Hard Shadow - Phong Lighting", (WIDTH, HEIGHT), vsync=True)
    canvas = window.get_canvas()
    gui    = window.get_gui()

    while window.running:
        with gui.sub_window("Phong Parameters", 0.02, 0.02, 0.30, 0.22):
